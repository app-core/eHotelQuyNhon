...
import java.io.*;
import java.util.*;
import elcom.com.util.*;
import com.elcom.Log.FileEvent;
import elcom.com.cfg.eLogger;
import org.apache.commons.net.ftp.FTPClient;
import elcom.com.core.common.*;
import elcom.com.core.*;
...
public class PMSCoreWrite {
     ...
     * @return String
     */
    public static String sendCmdRequestToPms(String command, int folioNum,
                                             int guestId) {
        String sendCmd = "";
        try {
     ...
     * @param command String
     * @param params Vector<SubProParam> : input=TranID,Output=Vector
     * @return String
     */
    public static String sendCmdAlertChargeToPms(String command, int transationID) {
        ...
        try {
            ...
            if (charge != null) {
                sendCmd = command + "|RN" + charge.getFolioNum() + "|GP" +
                          charge.getPinCode() +
                          "|P#" + charge.getTranID() + "|TA" + charge.getAmount() +
                          "|PTIPTV " + charge.getDesciption() +
                          "|DA" + charge.getTranDate() + "|TI" +
                          charge.getTranTime() + "|CU" + charge.getAmountUnit() +
                          "|R#"+charge.getIdReservation()+ "|" ;//tam rem phan R# ,day theo dung file chuan interface
                uploadFile(sendCmd);
            }
        } catch (Exception ex) {
            eLogger.writeLog(log,ex.getMessage());
    ...
    public static String sendCmdCommonToPms(String command,int objId){
       ...
       try {
           ...
           if(command!=null && command.equals("XM")){
               ...
               if (common != null) {
                   sendCmd = command + "|RN" + common.getFolioNum() + "|MI" +
                             common.getCmdId() + "|G#" + common.getClientId() +
                             "|MS2|DA" + common.getDate() + "|TI" + common.getTime() + "|";
                uploadFile(sendCmd);
               }
           }
   ...
   public static void sendCommand(String content){
    ...
    private static void uploadFile(String sendCmd){
      try {
     /* Kiem tra file(pms.ftp) cua Iptv co dang ton tai( hay chua duoc xu ly) tren FTP server?
          Neu file chua duoc xu ly thi ghi tiep theo vao file. Nguoc lai tao moi
          */
